...
class CareerKnowledgeBase:
    def __init__(self):
        ...
        try:
            self.collection.drop_index("career_text_index")
        except Exception as e:
            print(f"Could not drop index: {str(e)}")
        ...
        try:
            self.collection.create_index([
                ...
                ("related_interests", TEXT),  # Changed from personality_fit
    ...
    def search_by_skills(self, skills: List[str], top_n: int = 5) -> List[Dict[str, Any]]:
        """Find careers matching given skills using text search."""
    ...
    def search_by_interests(self, interests: List[str], top_n: int = 5) -> List[Dict[str, Any]]:
        """Find careers matching given interests using text search."""
        search_terms = " ".join(interests)
        return list(self.collection.find(
            {"$text": {"$search": search_terms}},
            {"score": {"$meta": "textScore"}}
        ).sort([("score", {"$meta": "textScore"})]).limit(top_n))
